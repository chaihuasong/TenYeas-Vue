...
export default {
  ...
  methods: {
    getData() {
      ...
      axios({
        method: "GET",
        url: this.serverUrl + "getAllHalfYearInfo",
        headers: {
        ...
        console.error('获取半年践行计划列表失败:', err)
    ...
    delUser(index) {
      ...
      this.$confirm('确认删除 id 为 ' + this.tableData[id].id + ' 的记录吗？')
          .then(() => {
            ...
            axios({
              method: "POST",
              url: this.serverUrl + "deleteHalfYearInfoById",
              data: data,
              headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
              }
